...
export async function postFoodAI(session, file) {
    try {
        ...
        const descriptionKo = parsed[1]?.replace(/`/g, '').replace(/\r?\n/g, '').trim()
        const originKo = parsed[2]?.replace(/`/g, '').replace(/\r?\n/g, '').trim()
        const howToEatKo = parsed[3]?.replace(/`/g, '').replace(/\r?\n/g, '').trim()
        const ingredientsKo = parsed[4]?.replace(/`/g, '').trim().split(/\r?\n/).map(i => i.replace(/^\*\s*/, '').trim()).filter(Boolean)
        const cantIngredientsKo = parsed[5]?.replace(/`/g, '').trim().split(/\r?\n/).map(i => i.replace(/^\*\s*/, '').trim()).filter(Boolean)
        const nameEn = parsed[6]?.replace(/`/g, '').replace(/\r?\n/g, '').trim()
