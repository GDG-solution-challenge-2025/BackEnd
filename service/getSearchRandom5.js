...
//repository
import {findSearchLandom5} from '../repository/findSearchLandom5.js'
import {findUser} from '../repository/findUser.js'

export async function getSearchRandom5(session) {
    try {
        ...
        const callFindSearchLandom5 = await findSearchLandom5()

        if (callFindSearchLandom5.result === false) {
            throw new Error()
        }

        if (callFindUser.lang === 0) {
            const foods = callFindSearchLandom5.nameKo.map(item => {
        ...
        const foods = callFindSearchLandom5.nameEn.map(item => {
