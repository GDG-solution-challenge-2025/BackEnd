...
export async function findSearchLandom10() {
    try {
        ...
        const query1 = 'SELECT nameKo, ANY_VALUE(imgURL) AS imgURL FROM search GROUP BY nameKo ORDER BY RAND() LIMIT 10'
        const [nameKo] = await callDbConnection.connection.execute(query1)
        const query2 = 'SELECT nameEn, ANY_VALUE(imgURL) AS imgURL FROM search GROUP BY nameEn ORDER BY RAND() LIMIT 10'
        ...
        console.log('[ERROR] findSearchLandom10')
        console.log(err + '\n\n')
