...
export async function findSearchLimt() {
    try {
        ...
        const query1 = 'SELECT nameKo, COUNT(*) as count  FROM search  GROUP BY nameKo  ORDER BY count DESC LIMIT 5'
        const [nameKo] = await callDbConnection.connection.execute(query1)
        const query2 = 'SELECT nameEn, COUNT(*) as count  FROM search  GROUP BY nameEn  ORDER BY count DESC LIMIT 5'
        ...
        console.log('[ERROR] findSearchLimt')
        console.log(err + '\n\n')
